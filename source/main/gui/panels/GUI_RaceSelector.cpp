...
/* cosmic vole - probably delete this - TODO how to handle skins with the mod?
...
*/
    ...
    if (m_loader_type == RT_Race)
    {
        //We need to load the races into the selector for the current championship
        ChampionshipManager& cm = ChampionshipManager::getSingleton();
        if (m_selected_entry != nullptr)
        {
            Ogre::String filename = m_selected_entry->fname;
            Ogre::String group = Ogre::ResourceGroupManager::getSingleton().findGroupContainingResource(filename);//cm.getName());
            if (Ogre::ResourceGroupManager::getSingleton().resourceExists(group, filename))
            {
                try
                {
                    ...
                    Ogre::DataStreamPtr ds = Ogre::ResourceGroupManager::getSingleton().openResource(filename, group);        
                    cm.loadChampionship(ds);
                    cm.setCacheEntry(m_selected_entry);
                }
                catch(...)
                {
        ...
        //TODO What to do if no championship currently selected? Show champ selector again?
        const std::vector<Race>& races = cm.getRaces();
        int i = 0;
        for (std::vector<Race>::const_iterator itr = races.begin(); itr != races.end(); itr++)
        {
            //Races probably won't use Categories at all
            Ogre::UTFString txt = ANSI_TO_UTF(itr->GetTrackName());//U("[") + TOUTFSTRING(++current_category) + U("/") + TOUTFSTRING(tally_categories) + U("] (") + TOUTFSTRING(num_elements) + U(") ") + title;
            /*m_Type*/m_Model->addItem(convertToMyGUIString(txt), i);
            i++;                    
        }
        int selRace = cm.getCurrentRaceIndex();
        if (selRace < 0)
            selRace = 0;
        if (selRace >= races.size())
            selRace = races.size() - 1;
        if (selRace >= 0)
        {
            try
            {
                /*m_Type*/m_Model->setIndexSelected(selRace);
                /*m_Type*/m_Model->beginToItemSelected();
            }
            catch (...)
            {
                return;
            ...
            //OnCategorySelected(*m_Type->getItemDataAt<int>(selRace));
            OnEntrySelected(*m_Model/*m_Type*/->getItemDataAt<int>(selRace));
        }
        return;        
    }
    ...
    for (std::vector<CacheEntry>::iterator it = entries->begin(); it != entries->end(); it++)
    {
        ...
        if (it->fext == "champ")
        {
            LOG("Found Championship: " + it->fname);
            if (m_loader_type == RT_Championship)
                add = true;
        }
        /*
        if (it->fext == "terrn2")
...
void CLASS::OnEntrySelected(int entryID)
{
    ...
    */
    ...
    {
        ChampionshipManager& cm = ChampionshipManager::getSingleton();
        cm.setSelectedRaceIndex(entryID);
        m_selected_entry = cm.getCacheEntry();
    }
    else
    {
...
void CLASS::OnSelectionDone()
{
    ...
    //New championship and race code - cosmic vole September 15 2017
    //This method is called when return is pressed or Enter Race / Enter Championship was clicked
    //When enter race is clicked, we need to call App::SetSimNextTerrain(terrainForRace); after setting the starting positions etc.
    if (m_loader_type == RT_Championship)
    {
        //Enter the championship and show the races
        
        //Get the current player truck
        
        this->Hide();
        this->Show(RT_Race);
        return;        
    }
    else if (m_loader_type == RT_Race)
    {
        ...
        */
        ...
        cm.setCurrentRaceIndex(raceIndex);
        
        Race *race = cm.getCurrentRace();
        if (race != nullptr)
        {
            Ogre::String terrainForRace = race->GetTerrainFileName();
            RoR::App::SetSimNextTerrain(terrainForRace);
            
            cm.setState(CS_RacePending);
            RoR::App::SetPendingAppState(App::APP_STATE_SIMULATION);
        }
        this->Hide(false);      
    }
    
    //Old code from main selector
    if (m_loader_type != LT_SKIN)
    {
        ...
        {
//TODO RaceLoaderType LT_SKIN doesn't exist. LT_Skin is a LoaderType            
//            Show(LT_SKIN);
        }
        else
        {
...
/* Vehicle specific stuff not used on our Race menu
    ...
    */
    ...
    {
        ChampionshipManager &cm = ChampionshipManager::getSingleton();
        std::vector<RaceCompetitor> standings = cm.GetSortedStandings();
        bool hasResult = false;
        Ogre::UTFString raceresulttxt = _L("┌────────────────────────────────────────────────────┐") + newline;
        raceresulttxt = raceresulttxt + _L("│Championship Standings                              │") + newline;
        raceresulttxt = raceresulttxt + _L("├─────────────────────────┬───────┬─────────┬────────┤") + newline;
        raceresulttxt = raceresulttxt + _L("│Driver                   │Points │Best Pos.│Podiums │") + newline;
        raceresulttxt = raceresulttxt + _L("├─────────────────────────┼───────┼─────────┼────────┤") + newline;
        int pos = 1;
        for (std::vector<RaceCompetitor>::iterator it = standings.begin(); it != standings.end(); ++it)
        {
            Ogre::UTFString driverName = TOUTFSTRING(pos) + Ogre::UTFString(". ");
            Ogre::UTFString name = it->GetName();
            //BUG !! This crashes. They both wrap char *s and append is taking a const char * that seems to be a tab character....????
            driverName.append(name);
            LOG("Competitor driver name is '" + name + "'.");
            driverName = (driverName.length() > 25) ? driverName.substr(0, 25) : driverName.append(25 - driverName.length(), ' ');
            Ogre::UTFString totalPoints = TOUTFSTRING(it->GetPoints());
            totalPoints = (totalPoints.length() > 7) ? totalPoints.substr(0, 7) : totalPoints.append(7 - totalPoints.length(), ' ');
            int iBestPos = it->GetBestPosition();
            if (iBestPos > 0)
            {
                hasResult = true;
            }
            Ogre::UTFString bestPos = (it->GetBestPosition() > 0) ? TOUTFSTRING(it->GetBestPosition()) : L"-";
            bestPos = (bestPos.length() > 9) ? bestPos.substr(0, 9) : bestPos.append(9 - bestPos.length(), ' ');
            Ogre::UTFString podiums = TOUTFSTRING(it->GetPodiums());
            podiums = (podiums.length() > 8) ? podiums = podiums.substr(0, 8) : podiums.append(8 - podiums.length(), ' ');
            raceresulttxt = raceresulttxt + L"│" + driverName + L"│" + totalPoints + L"│" + bestPos +  L"│" + podiums + "│" + newline;            
            pos++;
        }
        if (!hasResult)
        {
            //No championship results to display yet, so throw it all away!
            raceresulttxt = _L("");
        }
        else
        {
...
void CLASS::Show(RaceLoaderType type)
{
    if (!m_selection_done)
        return;
